...
1. **Remote MCP (preferred)** — set ``ESTORE_MCP_URL`` (or the generic
   ``HANDAAS_MCP_URL``) to a streamable-http endpoint. A token may be supplied
   via ``ESTORE_MCP_TOKEN`` / ``HANDAAS_MCP_TOKEN``.
2. **Local MCP (fallback)** — when no URL is configured, the client boots the
   local ``handaas-mcp-server/estore-mcp-server/server/mcp_server.py`` over
...
def _require_connection() -> Dict[str, Any]:
    ...
    if not conn:
        raise ConfigError(
            "未配置 MCP 连接。请设置环境变量 ESTORE_MCP_URL（Remote MCP streamable-http），"
